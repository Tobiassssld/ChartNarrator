...
OUTPUT_DIR = str(PROJECT_ROOT / "data" / "finetune_noanchor_4_5")
...
def prepare_entry(entry: Dict, image_dir_name: str) -> Dict:
    """Return one no-anchor fine-tuning entry.

    The user prompt keeps the original Stage 2 prompt only. No [CHART CONTEXT]
    is injected in this ablation condition.
    """
    ...
    return {
        ...
        "conversations": [
            {"from": "user", "value": original_prompt},
...
def prepare_noanchor_dataset(
    ...
    """Prepare train, validation, and test JSON files for the no-anchor ablation."""
    ...
    for p, label in [(input_path, "Input JSON"), (image_src, "Image dir")]:
    ...
    print("\n" + "=" * 72)
    print("  STAGE-2 VLM NO-ANCHOR DATASET PREPARATION")
    print("=" * 72)
    print(f"Input    : {input_path}")
    print(f"Images   : {image_src}  ->  {image_dst}")
    print(f"Output   : {output_dir}")
    print("Note     : anchor text NOT injected (no-anchor ablation)\n")
    ...
    for split_name, raw_list in splits:
        ...
        for entry in raw_list:
            ...
            prepared.append(prepare_entry(entry, "images"))
    ...
    summary_lines = [
        "NO-ANCHOR ABLATION DATASET SPLIT SUMMARY",
        "=" * 50,
        ...
        "Anchor        : NOT INJECTED (no-anchor ablation)",
        "",
...
def parse_args(argv: Optional[List[str]] = None):
    ...
    parser = argparse.ArgumentParser(
        description="Prepare the Stage 2 VLM no-anchor ablation dataset."
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    """Run no-anchor fine-tuning dataset preparation."""
    args = parse_args(argv)
    return prepare_noanchor_dataset(
