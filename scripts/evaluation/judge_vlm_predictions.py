...
Run from the repository root:
    ...
    python scripts/evaluation/judge_vlm_predictions.py --resume
    python scripts/evaluation/judge_vlm_predictions.py --debug --debug-limit 20
"""
...
# Route-conditional WA thresholds (metadata flag only; not a filter here).
WA_THRESHOLD = {
    "simple":  3.5,
    "complex": 4.5,
}
...
DEFAULT_INPUT_JSON = str(
    PROJECT_ROOT / "data" / "evaluation" / "predictions" / "predictions_test_stage2_4_5.json"
)
DEFAULT_OUTPUT_JSON = str(
    PROJECT_ROOT / "data" / "evaluation" / "evaluated_predictions" / "evaluated_predictions_test_stage2_4_5_primary.json"
)
...
def configure_gemini(api_key: str, model_name: str = JUDGE_MODEL_NAME):
    ...
    if not api_key:
        raise ValueError("GOOGLE_API_KEY is required for prediction judging.")
...
def find_image_path(json_image_field: str, image_root: str) -> Optional[str]:
    """
    Resolve Colab Drive path to local IMAGE_ROOT_DIR.
    Tries: basename only, then original path, then lowercased basename.
    """
    basename = os.path.basename(json_image_field.replace("\\", "/"))

    candidates = [
        os.path.join(image_root, basename),
        json_image_field,
        os.path.join(image_root, basename.lower()),
    ]
    for c in candidates:
        if os.path.isfile(c):
            return c
...
def judge_vlm_predictions(args) -> int:
    """Judge prediction JSON entries using the original Stage 2 judge logic."""
    try:
    ...
    for entry in tqdm(entries_to_process, desc="Judging predictions"):
        ...
        if not img_path:
            ...
            entry["prediction_evaluation"] = {"error": f"image not found: {entry.get('image')}"}
            evaluated_data.append(entry)
            continue

        img_bn = os.path.basename(entry.get("image", "").replace("\\", "/"))
        anchor_data = chart_anchors.get(img_bn) or chart_anchors.get(entry_id, {})
        ...
        pred_result = evaluate_with_exponential_backoff(
            img_path, prediction_text, anchor_text,
            route_label=route_label,
            morphology_family=morphology_family,
        ...
        if args.score_reference and reference_text:
            ...
            ref_result = evaluate_with_exponential_backoff(
                img_path, reference_text, anchor_text,
                route_label=route_label,
                morphology_family=morphology_family,
                model_client=model_client,
            )
...
def parse_args(argv: Optional[List[str]] = None):
    ...
    parser = argparse.ArgumentParser(description="Judge fine-tuned VLM predictions on Stage 2.")
    ...
    parser.add_argument("--images", default=DEFAULT_IMAGE_ROOT, help="Root directory for chart images.")
...
def main(argv: Optional[List[str]] = None) -> int:
    """Run Stage 2 VLM prediction judging."""
    args = parse_args(argv)
    return judge_vlm_predictions(args)
